...
Status VersionSet::LogAndApply(VersionEdit* edit, port::Mutex* mu) {
  ...
  // Unlock during expensive MANIFEST log write
  {
    ...
    if (s.ok()) {
      ...
      if (!s.ok()) {
        xLog(options_->info_log, 3, "MANIFEST write: %s", s.ToString().c_str());
      }
    }
    ...
    if (s.ok() && !new_manifest_file.empty()) {
      if (!options_->rotating_manifest) {
        ...
        for (size_t i = 0; i < 2; i++) {
          xLog(options_->info_log, 3, "Delete %s", names[i].c_str());
...
Status VersionSet::Recover() {
  ...
  if (env_->FileExists(curr.c_str())) {
    ...
    if (!s.ok()) {
      xLog(options_->info_log, 3, "CURRENT read: %s", s.ToString().c_str());
      if (status.ok()) {
        status = s;
      }
    }
  ...
  for (size_t i = 0; i < 3; i++) {
    if (!manifests[i].empty()) {
      ...
      if (!s.ok()) {
        xLog(options_->info_log, 3, "MANIFEST read: %s", s.ToString().c_str());
        if (status.ok()) {
          status = s;
        }
      }
